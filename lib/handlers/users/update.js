...
function handler (req, res, next) {
  var profile = { };
  console.log('INCOMING UPDATE', req.params, req.body, req.files);
  ...
  keys.forEach(install(valid, req.profile.user))
  ...
  function saveUpload (err, ref, hash) {
    ...
    if (!err && ref) {
      ...
      // es.pipeline(input, git, es.writeArray(dont));
      es.pipeline(input, git, es.writeArray(finish));
      function dont (err, results) {
          var body = { result: results.pop( )
            , full: req.profile
            , old: req.profile.user
            , 'new': profile.user
            , complete: profile
            };
          res.send(201, body);
    ...
    // TODO: info leak?
    res.send(405, {err: err, msg: ref, url: req.profile.url});
    next( );
  };
}
var endpoint = {
...
module.exports = function configure (opts, server) {
  ...
  console.log("UPDATE MIDDLE", endpoint.middleware);
