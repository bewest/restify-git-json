...
var Fetch = require('./default-fetch'); 
...
function register (opts, server) {
  ...
  server.events.on('update.profile', function onProfile (hook, next) {
    ...
    hook.map(function each (data, fetch) {
      ...
      function begin (err, profile) {
        console.log("UPDATE PROFILE", 'found user', profile || err);
        profile = helpers.applyDelta(profile || err, update);
        fetch(null, profile);
      }
    });
