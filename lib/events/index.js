...
function Stream (emitters, opts) {
  ...
  function writer (chunk) {
    ...
    emitters.emit(chunk.name, chunk, next);
...
Events.prototype.user = function user (name, cb) {
  ...
  params.args = [name];
  hook.stream.write.apply(hook.stream, params.args);
  hook.stream.end( );
...
if (!module.parent) {
  ...
  ev.on('user', function (hook, next) {
    console.log('every user request', hook.value( ));
    hook.stream.pipe(es.writeArray( function (err, result) {
      console.log('found result', result);
      return next( );
    }));
  });
  ev.user('bewest', function fetch (value) {
    console.log('modify value here', value);
  });
}
