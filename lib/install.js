...
function install (server, opts) {
  ...
  server.get('/repos/:owner/:repo/git/refs', middleware, function (req, res, next) {
    req.repo.listRefs("refs/", function (err, refs) {
      res.send(200, {err: err, body: refs});

    });
  });

  server.get('/repos/:owner/:repo/git/refs/:ref', middleware, function (req, res, next) {
    var ref = req.params.ref == 'HEAD' ? 'HEAD' : 'refs/' + req.params[0];
    console.log('ref', ref);
  ...
  server.get('/repos/:owner/:repo/git/commits', middleware, function (req, res, next) {
    res.send("ha");
  });
