...
function install (server, opts) {
  ...
  server.post('/repos/:owner/:repo/info/refs', gitMiddleWare, function (req, res, next) {
    console.log("req.repo", req.url, req);
    next( );
  });
    ...
    if (req.params.service) {
      ...
      if (service == 'git-upload-pack') {
        ...
        req.repo.listRefs("refs/", function (err, refs) {
          console.log('REFS', err, refs);
          var keys = Object.keys(refs);
          keys.forEach(function (ref, i) {
            var item = refs[ref] + " " + ref;
            if (i == 0) {
              firstref = false;
              item = item + '\000multi_ack side-band side-band-64k ofs-delta shallow no-progress include-tag multi_ack_detailed no-done';
            }
            if (i == keys.length - 1) {
              item = item + '^{}';
            }
